...
| 2.2 | IP addresses, DNS, ports — how computers find each other | ✅ Done | IPv4 vs IPv6, private vs public IPs, NAT, DNS resolution chain (resolver → root → TLD → authoritative), DNS record types (A, AAAA, MX, NS, TXT, CNAME), ports 0–65535, well-known ports, TCP vs UDP. Mini-project: dns_port_detective.sh — queries DNS records and scans common ports on a target domain. Ran against github.com — found ports 22, 80, 443 open; all database ports closed. Fixed Mac BSD nc timeout bug (-G 1 flag). Post-topic deep dive: every open port is a potential entry point — defence in depth (Fail2Ban, SSH keys only, rate limiting, WAF, monitoring). Which ports to restrict vs open to world. How to audit open ports with lsof and nmap. Ran nmap -sV against github.com — decoded fingerprint output, version hiding (SSH-2.0-9adb99f), Strict-Transport-Security header, 997 filtered ports, GitHub Matrix easter egg. Installed nmap via brew. Security lens: IP spoofing, DNS poisoning, port scanning, principle of least exposure, defence in depth, attack surface minimisation. AI lens: DNS load balancing used by AI API providers at scale. |
...
- **Last completed topic:** 2.2 — IP addresses, DNS, ports — how computers find each other
- **Next topic:** 2.3 — What is "the cloud" — really
- **Phase:** 2 — Networking + Cloud Fundamentals
- **Total topics completed:** 9 of 67

## Current Skill Snapshot

A quick read for any session to orient fast, without scanning the full topic table.

- **Comfortable with:** Terminal navigation (pwd, ls, cd and variants), file/folder structure, absolute vs relative paths, basic permission errors, navigating the tracker app's own codebase at a surface level. Shell scripting fundamentals — shebang, variables, command substitution $(), echo, chmod +x, nano, running scripts with ./. Process inspection and control — ps aux, ps aux -r, ps aux -m, top, kill, jobs, background processes with &. Reading ps aux output columns. Mac vs Linux syntax differences in ps and wc. File permissions — owner/group/others, rwx, chmod numeric system (600, 755), ls -l permission string decoding, root/sudo, principle of least privilege, find command with -perm flags. Networking fundamentals — packets, routing, hops, private vs public IP ranges, DNS, HTTPS and its limitations, traceroute output reading. IP addressing — IPv4 vs IPv6, NAT, address exhaustion, dual-stack. DNS resolution chain — recursive resolver, root server, TLD server, authoritative server, TTL. DNS record types — A, AAAA, MX, NS, TXT, CNAME. Ports — well-known ports table, TCP vs UDP, port scanning with nc and nmap, lsof for local port audit, reading nmap -sV fingerprint output. Security — attack surface, defence in depth, Fail2Ban, SSH key-only auth, WAF, rate limiting, HSTS, version hiding, principle of least exposure.
- **In progress:** Nothing currently in progress — Topic 2.2 fully complete. Starting Topic 2.3 next.
- **Not yet covered:** All of cloud, Python, Git/GitHub fundamentals, Docker, CI/CD, Terraform/Kubernetes, monitoring/MLOps.
...
| 11 | 09 Jul 2026 | 2.2 — IP addresses, DNS, ports | IPv4 vs IPv6, address exhaustion, NAT, private IP ranges, DNS resolution chain, DNS record types (A/AAAA/MX/NS/TXT/CNAME), ports, well-known ports table, TCP vs UDP. Built dns_port_detective.sh — ran against github.com, found ports 22/80/443 open, all database ports closed. Fixed nc timeout issue on Mac (added -G 1 flag). Post-topic: attacker port targeting, defence in depth (Fail2Ban/SSH keys/WAF/rate limiting/monitoring), lsof for local port audit, nmap -sV deep scan. Decoded nmap fingerprint output against github.com — version hiding, HSTS header, 997 filtered ports, Matrix easter egg. Installed nmap via brew. |
...
| 09 Jul 2026 | Noticed the port scan was hanging for over 2 minutes and flagged it immediately rather than waiting indefinitely | Debugging instinct — recognising when something is wrong vs just slow, and calling it out |
| 09 Jul 2026 | Asked what GitHub's Azure IP and Microsoft MX records mean — connected the DNS output back to real infrastructure decisions | Systems thinking — reading DNS records as a window into how a company's infrastructure is actually built |
| 09 Jul 2026 | Asked which specific ports attackers mainly target and whether ports 22/80/443 found in the scan can actually be used for attacks | Security depth — not accepting "ports are open" as a neutral fact, immediately asking what the real-world attack implications are |
| 09 Jul 2026 | Asked as a DevOps/cloud engineer which ports to restrict to IP, how to prevent attacks on accidentally open ports, and how to check if own ports are hackable | Systems thinking — connected the theory directly to operational responsibility, asking all three angles at once: restrict, prevent, detect |
| 09 Jul 2026 | Asked what the nmap fingerprint output meant after seeing the raw SF: block — didn't just accept confusing output, wanted to understand it | Debugging instinct — questioning unfamiliar output rather than ignoring it |
...
*Last updated: 09 Jul 2026 — Topic 2.2 complete + post-topic deep dive. Port attack vectors (22/80/443), defence in depth, lsof, nmap -sV, decoded GitHub nmap fingerprint output, HSTS, version hiding, 997 filtered ports, Matrix easter egg. Installed nmap via brew. 4 new mindset moments added. Next: 2.3 — What is the cloud. Total: 9 of 67.*
